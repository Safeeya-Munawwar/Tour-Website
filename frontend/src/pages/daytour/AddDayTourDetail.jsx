...
export default function AddDayTourDetail() {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    accept: { "image/*": [] },
    onDrop: (files) => setFormData({...formData, heroImageFile: files[0], heroImagePreview: URL.createObjectURL(files[0])})
  });
    ...
    <div className="flex">
      ...
      <div className="ml-64 flex-1 p-6">
        <form onSubmit={handleSubmit} className="bg-white p-8 shadow rounded max-w-3xl mx-auto">
          ...
          {/* Hero Image */}
          <div {...getRootProps()} className="border-2 border-dashed p-6 mb-3 cursor-pointer">
            <input {...getInputProps()}/>
          ...
          <input className="border p-3 w-full mb-3" placeholder="About Paragraph 1" value={formData.aboutParagraphs[0]} onChange={e=>{
            const p = [...formData.aboutParagraphs]; p[0]=e.target.value; setFormData({...formData,aboutParagraphs:p});
          }}/>
          <input className="border p-3 w-full mb-3" placeholder="About Paragraph 2" value={formData.aboutParagraphs[1]} onChange={e=>{
            const p = [...formData.aboutParagraphs]; p[1]=e.target.value; setFormData({...formData,aboutParagraphs:p});
          }}/>

          <input className="border p-3 w-full mb-3" placeholder="History Title" value={formData.historyTitle} onChange={e=>setFormData({...formData,historyTitle:e.target.value})}/>
          <input className="border p-3 w-full mb-3" placeholder="History Left List (comma separated)" value={formData.historyLeftList.join(",")} onChange={e=>{
            setFormData({...formData,historyLeftList:e.target.value.split(",")});
          }}/>
          <input className="border p-3 w-full mb-3" placeholder="History Right List (comma separated)" value={formData.historyRightList.join(",")} onChange={e=>{
            setFormData({...formData,historyRightList:e.target.value.split(",")});
          }}/>
