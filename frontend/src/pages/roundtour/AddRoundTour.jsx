...
export default function AddRoundTour() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // debug
      console.log("Tour Card file:", formData.imgFile);
      for (let pair of tourData.entries()) {
      ...
      // debug detail data entries (files won't print fully)
      for (let pair of detailData.entries()) {
        console.log("detail", pair[0], pair[1]);
      }
      ...
      alert("Round Tour added successfully!");
      navigate("/admin/round-tours");
    } catch (err) {
      console.error(err);
      alert("Error adding round tour: " + (err.response?.data?.error || err.message));
    }
  };
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 ml-64 p-6 bg-gray-50">
        <RoundTourForm formData={formData} setFormData={setFormData} handleSubmit={handleSubmit} submitLabel="Add Round Tour" />
      </div>
    </div>
  );
}
